...
async function handleApi(req, res, url) {
  ...
  if (method === "POST" && pathname === "/api/login") {
    ...
    if (!account || !verifyPassword(password, account.passwordSalt, account.passwordHash)) {
      return json(res, 401, { error: requestedRole === "tester"
        ? "No active tester account matches this email and password. Ask the QA Manager to reset the password, then try again on this same QAGarden URL."
        : "Incorrect manager email address or password." });
    }
    await createSession(res, account);
